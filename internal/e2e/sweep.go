...
// LogFn is the minimal logger contract Sweep needs. Tests pass t.Logf;
// TestMain (which has no *testing.T) passes log.Printf.
type LogFn func(format string, args ...any)

// Sweep removes leftover e2e-prefixed resources from the sandbox. It is
// best-effort: every API failure is sent to logf and the sweep continues.
//
// In this scaffolding PR only the contacts list is swept; the other
// resource types are stubbed with TODOs for follow-up PRs.
func Sweep(logf LogFn, h *Harness) {
	if h == nil {
		return
	}
...
// organisation_name (or first/last name) starts with fixtureMarker. Soft-
// deletes that FreeAgent translates into hide-rather-than-purge are fine —
// hidden contacts don't appear in the default listing.
func sweepContacts(ctx context.Context, logf LogFn, h *Harness) {
	const path = "/v2/contacts?per_page=100"

	body, status, _, err := h.Client.Do(ctx, http.MethodGet, path, nil, "")
	if err != nil {
		logf("e2e sweep: list contacts: status=%d err=%v", status, err)
		return
	}
	...
	if err := json.Unmarshal(body, &page); err != nil {
		logf("e2e sweep: decode contacts: %v", err)
		return
	}
	...
	for _, c := range page.Contacts {
		...
		if _, dstatus, _, derr := h.Client.Do(ctx, http.MethodDelete, c.URL, nil, ""); derr != nil {
			logf("e2e sweep: DELETE %s -> %d: %v", c.URL, dstatus, derr)
		}
	}
}
